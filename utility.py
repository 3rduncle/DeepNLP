...
def embedding_layer_weights(vocabulary, word2vec, dim=300, padding=False):
    if padding:
        pad = [np.random.uniform(-0.25,0.25,dim)]
    else:
        pad = []
    embedding_weights = np.array(pad + [word2vec.get(w, np.random.uniform(-0.25,0.25,dim)) for w in vocabulary])
